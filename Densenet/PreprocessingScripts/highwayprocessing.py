...
# x,l = proc.frame2label_train_datagen(size = (256,256)  ,  batch_size = 16 , validation = True , normalise = True)
# x,f,fl = proc.frame2futurelabel_train_datagen(size=(256,256) , batch_size = 16 , time_step = 1 , normalise = True)
...
image_future = None
label_future = None
model.load_weights("models/newDensenetEpoch10.h5")

for e in range(11,epochs+1):
    for x , l in proc.frame2label_train_datagen(size=(256,256) , batch_size = 80 , normalise = True):
        image , label = x[:1] , l[:1] 
        history = model.fit(x, l, batch_size=4, epochs=1,verbose=1) 
        print("Loss :",history.history['loss'] , " Jaccard Distance :", history.history['jaccard_distance'])
        
    if (e%10==0):
        model.save_weights("models/newDensenetEpoch{}.h5".format(e))
        print("model saved")
    
    if (e%10==0):
        pred = model.predict(image)
        pred_classm = np.argmax(pred[0] , axis = -1)
        pred_label = proc.class2image(pred_classm)
        label_classm = np.argmax(label[0] , axis = -1)
        ...
        figure.savefig("results/densenet_%d.png" % (e))
        print("image saved")
        plt.close()
